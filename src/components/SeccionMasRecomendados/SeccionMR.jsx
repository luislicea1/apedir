import React, { lazy, Suspense } from "react";
import TituloDeSeccion from "../Seccion/TituloDeSeccion";
import ListadoDeComponentesMR from "./ListadoDeComponentesMR";

const TituloDeSeccion = lazy(() => import("../Seccion/TituloDeSeccion"));
const ListadoDeComponentesMR = lazy(() => import("./ListadoDeComponentesMR"));

export default function SeccionMR(props) {
  const container = {
    marginTop: "30px",
  };

  const section = {
    width: "100%",
    maxWidth: "1024px",
    marginTop: "20px",
    display: "flex",
    flexDirection: "column",
    justifyContent: "center",
  };

  return (
    <div
      style={container}
      className="container flex z-40 w-full h-auto items-center justify-center data-[menu-open=true]:border-none  top-0 inset-x-0   backdrop-blur-lg data-[menu-open=true]:backdrop-blur-xl backdrop-saturate-150 bg-background/70"
    >
      <section className="section px-6" style={section}>
        <Suspense>
          <TituloDeSeccion title={props.title}></TituloDeSeccion>
          <ListadoDeComponentesMR
            lugares={props.lugares}
          ></ListadoDeComponentesMR>
        </Suspense>
      </section>
    </div>
  );
}
